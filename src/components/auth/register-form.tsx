...
export function RegisterForm() {
  ...
  const initialState: RegisterFormState = { errors: {}, message: null, success: false };
  ...
  return (
    <Card className="w-full mx-auto border border-border shadow-sm">
      <CardHeader className="space-y-1 pb-6">
        <CardTitle className="text-xl sm:text-2xl font-semibold tracking-tight text-center">Create an Account</CardTitle>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="Your Name" autoComplete="name" className="focus:ring-1 focus:ring-primary" {...field} />
                  </FormControl>
                  {state.errors?.name && <FormMessage className="text-destructive">{state.errors.name.join(", ")}</FormMessage>}
                  <FormMessage />
                </FormItem>
              )}
            />
                ...
                <FormItem>
                  ...
                  <FormControl>
                    <div className="relative">
                      <Input placeholder="••••••••" type={showPassword ? "text" : "password"} autoComplete="new-password" className="focus:ring-1 focus:ring-primary pr-10" {...field} />
                      <Button
                        type="button"
                        variant="ghost"
                        size="icon"
                  ...
                  <FormDescription className="text-xs text-muted-foreground mt-1">8+ characters with uppercase, lowercase, number, and special character</FormDescription>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <div className="relative">
                      <Input placeholder="••••••••" type={showConfirmPassword ? "text" : "password"} autoComplete="new-password" className="focus:ring-1 focus:ring-primary pr-10" {...field} />
                      <Button
                        type="button"
                        variant="ghost"
                        size="icon"
                  ...
                  {state.errors?.confirmPassword && <FormMessage className="text-destructive">{state.errors.confirmPassword.join(", ")}</FormMessage>}
                  <FormMessage />
                </FormItem>
              )}
            />
            ...
            <div className="flex items-center justify-between">
              <Button type="button" variant="link" className="px-0 text-sm font-medium text-primary underline-offset-4 hover:underline" onClick={() => setShowPassword((prev) => !prev)}>
                {showPassword ? "Hide password" : "Show password"}
              </Button>
              <Button type="button" variant="link" className="px-0 text-sm font-medium text-primary underline-offset-4 hover:underline" onClick={() => setShowConfirmPassword((prev) => !prev)}>
      ...
      <CardFooter className="flex flex-col space-y-4 pt-0 px-6 pb-6">
        ...
        <div className="text-center pt-2">
          <p className="text-sm text-muted-foreground">
            ...
            <Link href="/auth/login" className="font-medium text-primary underline-offset-4 hover:underline transition-colors">
              Sign in
            </Link>
          </p>
        </div>
